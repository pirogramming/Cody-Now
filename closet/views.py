...
def test_gen_cody(request):
    ...
    try:
        ...
        if request.user.is_authenticated:
            try:
                gender_val = request.user.get_gender_display() if hasattr(request.user, 'gender') else "미지정"
            except AttributeError:
                gender_val = "미지정"
            user_info = {
                'gender': gender_val,
                'age': f"{request.user.age}세" if hasattr(request.user, 'age') and request.user.age is not None else "미지정",
                'height': f"{request.user.height}cm" if hasattr(request.user, 'height') and request.user.height is not None else "미지정",
                'weight': f"{request.user.weight}kg" if hasattr(request.user, 'weight') and request.user.weight is not None else "미지정",
                'style': request.user.get_style_display() if hasattr(request.user, 'style') else "미지정",
            }
        else:
            # 익명 사용자는 클라이언트에서 전달한 gender 값(또는 기본값 "미지정")을 사용
            user_info = {
                'gender': data.get('gender', "미지정"),
                'age': "미지정",
                'height': "미지정",
                'weight': "미지정",
                'style': "미지정",
...
위 정보를 고려하여 적합한 코디를 추천해주세요.
반드시 무신사 스탠다드 제품으로 추천하고, 출력 형식을 markdown으로 제공해주세요.
        """
        
        chat_session = model.start_chat()
        response_message = chat_session.send_message(prompt)
        ...
        if response_message and response_message.text:
            ...
            RecommendationResult.objects.create(
                user=request.user if request.user.is_authenticated else None,
                outfit=outfit,
                original_text=response_message.text,
        ...
        logger.error(f"Error in test_gen_cody: {str(e)}", exc_info=True)
        return JsonResponse({"error": str(e)}, status=500)
