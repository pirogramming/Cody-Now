...
def get_first_custom_search_image_info(query, api_key=API_KEY, search_engine_id=SEARCH_ENGINE_ID):
    ...
    매개변수:
      - query: 검색어 (예: "무신사 스탠다드 릴렉스드 히든 밴딩 크롭 데님 팬츠")
      - api_key: API 키
      - search_engine_id: 검색 엔진 ID (cx)

    반환:
      - page_link: 이미지 결과를 클릭했을 때 이동되는 페이지 URL (실제 제품 상세 페이지)
      - image_url: 이미지의 직접 URL
    """
...
def update_product_links(markdown_text, api_key=API_KEY, search_engine_id=SEARCH_ENGINE_ID):
    """
    마크다운 내 "[제품명](링크) - ..." 패턴을 찾아,
    각 제품에 대해 Custom Search API로 검색한 결과의 실제 구매 페이지 링크와 이미지 URL을 이용해 HTML 스니펫으로 치환합니다.
    """
    # 수정된 정규식: 제품명은 대괄호, 링크는 소괄호 안에 있고, 그 뒤에 '-'로 시작하는 설명이 붙음
    ...
    def repl(match):
        ...
        new_link, new_img = get_first_custom_search_image_info(query, api_key, search_engine_id)
